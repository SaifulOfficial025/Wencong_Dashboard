...
export const SalesProvider = ({ children }) => {
	...
	const fetchAgents = async (opts = {}) => {
		...
		try {
			...
			else if (json.totalCount !== undefined)
	...
	const uniqueAgentNames = Array.from(
		new Set(agents.map((a) => a.companyName).filter(Boolean))
	);
	const uniqueAgentCodes = Array.from(
		new Set(agents.map((a) => a.code).filter(Boolean))
	);
	const uniqueAgentGroups = Array.from(
		new Set(
			agents
				.map((a) =>
					a.agentGroupId ? `Group ${a.agentGroupId}` : a.agentGroupId
				)
				.filter(Boolean)
		)
	);
	...
	return (
		<SalesContext.Provider
			value={{
				...
				setPerPage: (pp) =>
					fetchAgents({ page: 1, perPage: pp, filters }),
				uniqueAgentNames,
				uniqueAgentCodes,
				uniqueAgentGroups,
				agentGroups,
