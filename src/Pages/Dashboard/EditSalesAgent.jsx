...
function EditSalesAgent() {
	...
	const onSubmit = async (data) => {
		try {
			...
			if (
				data.newPassword &&
				String(data.newPassword).trim().length > 0
			) {
			...
			if (selectedFile) {
				...
				if (!res.ok) {
					// eslint-disable-next-line no-console
					console.debug(
						"[EditSalesAgent] multipart PUT failed with status:",
						res.status
					);
			...
			// eslint-disable-next-line no-console
			console.debug(
				"[EditSalesAgent] response status:",
				res.status,
				json
			);
			if (res.ok) {
				...
				reset({
					...
					status: returned.status
						? String(returned.status).toLowerCase()
						: "",
							...
							<div>
								...
								<div className="relative">
									...
									>
										...
										{initialAgent &&
										initialAgent.accountBook ? (
											<option
												value={initialAgent.accountBook}
											>
												{initialAgent.accountBook}
											</option>
										) : (
								...
								<div
									className={`relative w-full ${
										initialAgent && initialAgent.file
											? "h-32"
											: "h-12"
									} bg-[#FFF0ED] border border-[#F24E1E] border-dashed rounded-md flex items-center justify-center cursor-pointer overflow-hidden py-2`}
								>
									{/* Hidden file input */}
